...
const BlogPostArchive = ({ posts = [], archiveTitle }) => {
  ...
  if (!posts || posts.length === 0) {
    ...
    return <div className='animate__animated animate__fadeInDown' ref={targetRef}>
      <div className='pt-16 pb-4 text-3xl dark:text-gray-300' id={archiveTitle}>{archiveTitle}</div>
      <ul>
        {posts.map(post => (
          <li key={post.id} className='mx-4 border-l-2 p-1 text-xs md:text-base items-center  hover:scale-x-105 hover:border-gray-500 dark:hover:border-gray-300 dark:border-gray-400 transform duration-500'>
