...
const BlogPostArchive = ({ posts = [], archiveTitle }) => {
  ...
  if (!posts || posts.length === 0) {
    ...
    return <div ref={targetRef}>
      ...
      <ul>
        {posts.map(post => (
          <li key={post.id} className='animate__animated animate__fadeInDown border-l-2 p-1 text-xs md:text-base items-center  hover:scale-x-105 hover:border-gray-500 dark:hover:border-gray-300 dark:border-gray-400 transform duration-500'>
