...
const Footer = ({ title }) => {
  ...
  return (
    ...
    >
      <FontAwesomeIcon icon={faCopyright} /> <span> {`${from} - ${y}`} | <a href={BLOG.link} className='font-bold text-gray-100'>{BLOG.author}</a> | Powered by <a target='_blank' rel='noreferrer' href='https://notion.so' className='font-bold text-gray-100'>❤</a></span>
