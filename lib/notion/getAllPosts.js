...
export async function getAllPosts({
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    properties.createdTime = new Date(
      pageBlock[id].value?.created_time
    ).toString()
    properties.lastEditedTime = new Date(
      pageBlock[id].value?.last_edited_time
    ).toString()
...
// Get the cover image from the Block; take the PageCover first, otherwise take the content image
